...
def hot_articles():
    ...
    return {
        'articles' :  Article.objects.published().annotate(
        count = Count('comments',filter= Q(comments__posted__gt= last_month) and 
                      Q(comments__content_type_id= content_type_id))
                        ).order_by('-count','-publish')[:2]
        ,
...
def change_class_side_bar(request, link_name, content, classes):
    return {
        "request": request,
        "link_name":link_name,
